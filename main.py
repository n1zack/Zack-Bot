...
# --- معالجة ملفات الأرشيف والجلسات (المحدثة بدقة شاملة لجميع أنواع الملفات) ---
...
async def handle_session_file(event):
    try:
        ...
        await event.respond("🔍 **جاري فحص جميع الملفات واستخراج الجلسات بالطريقة الشاملة...**")
        success_numbers = []

        # البحث الشامل في كل المجلدات الفرعية
        for root, dirs, files in os.walk(extract_dir):
            for file in files:
                f_path = os.path.join(root, file)
                session_str = ""
                
                # تخطي الملفات غير المرغوبة
                if file.startswith('.') or file.lower() == 'readme.txt' or file.startswith('info_'):
                    continue

                try:
                    # الحالة الأولى: ملف قاعدة بيانات .session (تتم قراءته عبر تليثون أو كـ SQLite مباشرة)
                    if file.endswith('.session') or file.endswith('.sqlite'):
                        try:
                            # محاولة فتحه مباشرة بواسطة تيليثون كملف جلسة
                            async with TelegramClient(f_path, API_ID, API_HASH) as temp_client:
                                if await temp_client.is_user_authorized():
                                    session_str = temp_client.session.save()
                        except Exception:
                            # إذا فشل، نقرأ جدول الجلسات يدوياً عبر sqlite3 ونحولها لـ StringSession
                            try:
                                ...
                                logger.error(f"SQLite fallback error on {file}: {sqlex}")

                    # الحالة الثانية: ملف نصي يحتوي على StringSession مباشرة أو بيانات الجلسة النصية
                    else:
                        with open(f_path, "r", encoding="utf-8", errors="ignore") as rf:
                            content = rf.read().strip()
                            if len(content) > 10:
                                # إذا كان الملف يحوي StringSession صريحاً
                                session_str = content

                    # التحقق من الجلسة المستخرجة وحفظ الرقم في Neon
                    if session_str:
                        try:
                        ...
                        except Exception as vcex:
                            logger.error(f"Auth validation error for session: {vcex}")
        ...
        if success_numbers:
            ...
            await event.respond("⚠️ **لم يتم العثور على أرقام صالحة.** يرجى التأكد من أن الأرشيف يحتوي على ملفات `.session` صحيحة أو ملفات نصية تحوي StringSession صالحة وليست تالفة.")
    except Exception as e:
